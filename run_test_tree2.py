...
def hierarchical_clustering_with_external_primary(
    ...
    for cluster_id in set(cluster_ids):
        ...
        for subcluster_id in range(0, num_subclusters):
            ...
            subsub_cluster_labels = fcluster(
                linked_subsub, num_subsubclusters, criterion="maxclust"
            )
            ...
            for subsubcluster_id in range(0, num_subsubclusters):
                final_indices = sub_indices[
                    np.where(subsub_cluster_labels == subsubcluster_id)[0]
                ]  # Correctly index into sub_indices
        ...
        if isinstance(cluster_data, list):  # Primary cluster directly
            ...
            if relevance == 1:
                ...
                if primary_indices:
                    ...
                    primary_distances = cosine_similarity(
                        primary_points, primary_centroid
                    )
                    if primary_distances.numel() > 0:
                        closest_primary_idx = torch.argmin(primary_distances).item()
                        closest_points_indices.append(
                            int(primary_indices[closest_primary_idx])
                        )
                continue
                    ...
                    if isinstance(subclusters, dict):  # Sub-subclusters
                        for subsubcluster_id, indices in subclusters.items():
                            ...
                            distances = cosine_similarity(
                                points_in_subsubcluster, subsubcluster_centroid
                            )
                            if distances.numel() > 0:
                                closest_idx_in_subsubcluster = torch.argmin(
                                    distances
                                ).item()
                                closest_global_idx = indices[
                                    closest_idx_in_subsubcluster
                                ]
                        ...
                        points_in_subcluster = x[
                            torch.tensor(subclusters, dtype=torch.long)
                        ]
                        subcluster_centroid = points_in_subcluster.mean(dim=0)
                        distances = cosine_similarity(
                            points_in_subcluster, subcluster_centroid
                        )
                        if distances.numel() > 0:
                            closest_idx = torch.argmin(distances).item()
...
def depth_expansion(**item):
    save_folder = "./outputs/nextqa_features"
    img_feats = load_frame_features(item["vid"], save_folder)
    relevance_scores = item["relevance"]
    cluster_ids_x = item["cluster_ids_x"]
    ...
    closest_points_temporal_subsub = find_closest_points_in_temporal_order_subsub(
        img_feats, clusters_info, relevance_scores
    )
    # print("closest_points_temporal_subsub",closest_points_temporal_subsub)
    return {"qid": item["qid"], "sub_cluster_ids": closest_points_temporal_subsub}


if __name__ == "__main__":
    output_path = "./outputs/0329/nextmc_gpt_4o_tree2.jsonl"
    input_path = "./outputs/0329/nextmc_gpt_4o_tree1.jsonl"
    run_task(
        depth_expansion,
