...
def update_relevance_response(text):
    ...
    if response is None:
        return None
    relevance_match = re.search(r"frame relevance: \[([0-9, ]+)\]", response, flags=re.IGNORECASE)
    if relevance_match:
...
async def run_task(data, narration, sem):
    async with sem:
...
async def build_tree(args, data, narration, sem):
    ...
    # load frame features
    frame_feats = load_frame_features(data["vid"], args.feature_path)
    ...
    while True:
        ...
        loc_caption = [
            captions[i] for i in tree_node if i > 0 and i <= len(captions)
        ]

        # Join the relevant captions with "narration" label
        narr = "\n".join([f"{i}: {loc}" for i, loc in enumerate(loc_caption)])
        prompt, pred = await run_task(data, narr, sem)
...
async def eval():
    args = HfArgumentParser(ExtractingArguments).parse_args_into_dataclasses()[0]
    args.max_cluster_num = 32
    args.init_cluster_num = 8
    args.iter_threshold = 4
    args.default_adpative_rate = 2

    eval_dataset = build_dataset(
        args.dataset_config, args.dataset_name, is_training=False
    )
    args.feature_path = './outputs/nextqa_features'
    output_path = "./outputs/nextqa_tree.jsonl"
    ...
    with jsonlines.open(output_path, "a") as writer:
        ...
        for data in eval_dataset:
            if data["qid"] in processed:
                continue
            narr = narr_data[data["vid"]]
            tasks.append(build_tree(args, data, narr, sem))
        for result in tqdm(asyncio.as_completed(tasks), total=len(tasks)):
            ...
            if result is not None:
                ...
                invalid += 1
        print(f"Invalid: {invalid}/{len(eval_dataset)}")
